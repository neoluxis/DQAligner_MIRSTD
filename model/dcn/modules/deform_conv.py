...
class DeformConv(ConvOffset2d):  
    def __init__(self, in_channels, out_channels,
        ...
        super(DeformConv, self).__init__(in_channels, out_channels,
                                  kernel_size, stride, padding, dilation, groups, deformable_groups, im2col_step, bias)
        out_channels = self.deformable_groups * 3 * self.kernel_size[0] * self.kernel_size[1]  # 3:每个采样点有两个偏移量（x和y方向）和一个调制标量
        self.conv_offset = nn.Conv2d(self.in_channels,
                                          out_channels,
                                          kernel_size=self.kernel_size,
                                          stride=self.stride,
    ...
    def forward(self, input, offset):
        ...
        offset = torch.cat((o1,o2), dim=1)  # 将x和y方向的偏移量拼接起来
        mask = torch.sigmoid(mask)  # 调制标量应用sigmoid激活函数
